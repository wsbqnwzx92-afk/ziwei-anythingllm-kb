...
- **五行局判定** - 根据命宫干支的纳音确定（标准规范）
- **十四主星排列** - 按五行局规则安排主星
- **辅星布置** - 安排左辅、右弼、文昌、文曲、禄存、天马等
- **四化星计算** - 根据年干计算化禄、化权、化科、化忌

### ✅ 知识库友好输出
- **Markdown格式** - 结构清晰，包含完整表格和通俗易懂的断语
- **JSON格式** - 结构化数据，便于程序化处理和备份
...
2. **学派规范**：本脚本遵循标准紫薇斗数学派规则
3. **参考用途**：排盘结果仅供参考学习，不构成任何决策依据
4. **准确性**：使用公式算法，部分细节可能需要农历转换进一步优化
